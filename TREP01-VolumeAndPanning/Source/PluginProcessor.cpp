...
Mu45effectAudioProcessor::Mu45effectAudioProcessor()
{
	...
	balance = new PluginParameter("pan", 0.0, -50.0, 50.0, "Pan", String::empty);
...
{
    volumeGain = pow(10.0, ((volumeParam->getActualValue()) / 20.0));
	leftGain = cos(((balance->getActualValue() + 50.0) / 100.0) * (3.14159265 / 2.0));
	rightGain = sin(((balance->getActualValue() + 50.0) / 100.0) * (3.14159265 / 2.0));
...
void Mu45effectAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    {
        left[samp] = (volumeGain * leftGain) * left[samp];
        right[samp] = (volumeGain * rightGain) * right[samp];
    }
}
